...
def main(argv):
    ...
    for e in (range(epochs) if not s.infinite_loop else count()):
        ...
        for i,(image,_) in enumerate(trainloader):
            ...
            loss_g=criterion(crit(unet_col)[:,0], ones)
            #backpropagation
            loss_g.backward()
            optimizer_g.step()
            ...
            real_loss=criterion(crit(image)[:,0],ones)
            #requires no gradient in unet col
            fake_loss=criterion(crit(unet_col.detach())[:,0],zeros)
            ...
            if (i+len(trainloader)*e)%report_freq==report_freq-1:
                print('Epoch %i, batch %i: \tunet loss=%.2e, \tcritic loss=%.2e'%(e+1,i+1,
                loss_g.item(),loss_c.item()))#np.convolve(loss_hist, np.ones((s.batch_size,))/s.batch_size, mode='valid')))
