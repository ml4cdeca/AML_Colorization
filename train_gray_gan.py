...
import sys, getopt
from models.discriminator import gray_critic
...
def main(argv):
    ...
    #define critic 
    if data_path == './cifar-10':
        crit=gray_critic(trainset.data.shape[1]).to(device)
    elif 'places' in data_path:
        crit=gray_critic(256).to(device)
    ...
    for e in (range(prev_epochs, prev_epochs + epochs) if not infinite_loop else count(prev_epochs)):
        ...
        for i,batch in enumerate(trainloader):
            ...
            fooling_loss=criterion(crit(torch.cat((X,unet_col),dim=1))[:,0], ones[:batch_size])
